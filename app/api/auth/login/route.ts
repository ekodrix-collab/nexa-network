...
export async function POST(req: Request) {
  try {
    ...
    // 1. Check if user exists (MOCKED FOR DEPLOYMENT)
    ...
    // MOCK LOGIN SUCCESS ALWAYS
    if (username !== 'admin' || password !== 'admin') {
      return NextResponse.json({ error: 'Invalid credentials (use admin/admin for now)' }, { status: 401 })
    }

    // 3. Generate token
    const token = signToken({ id: 'mock-id', username: 'admin' })
