...
export default function CTA({ settings }: { settings?: any }) {
  const s = settings || {}
  const ctaTitle = s.ctaTitle || s.cta_title || "Let's build a smarter, safer future together."
  const ctaDescription = s.ctaDescription || s.cta_description || "Partner with Nexa Network Solutions to transform your enterprise IT infrastructure with Qatar's leading systems integrator."
  const ctaBgImage = s.ctaBgImage || s.cta_bg_image || "/images/cta-bg.jpg"
